...
class TelegramController {
  ...
  /**
   * Forward error payload to n8nTrackingUrl when Gemini fails
   */
  private async forwardErrorToN8N(
    chatId: number,
    userId: string,
    ...
    if (!config.n8nTrackingUrl || config.n8nTrackingUrl.trim() === "") {
      logger.warn(
        `[User ${userId}] N8N tracking URL not configured, cannot forward error payload`
      );
      return;
    }
    ...
    try {
      ...
      await sendWebhookWithRetry(
        ...
        `${taskType} error (N8N tracking)`,
        chatId
      );
      logger.info(
        `[User ${userId}] Error payload forwarded successfully to N8N`
      );
    } catch (forwardError: any) {
      logger.error(
        `[User ${userId}] Failed to forward error payload to N8N: ${forwardError.message}`
      );
      // Don't throw - we've logged the error, continue silently
    }
  }
  ...
  async handleGenerate(msg: TelegramBot.Message): Promise<void> {
    ...
    if (!rateLimit.allowed) {
      await telegramService.sendMessage(
        chatId,
        telegramService.formatRateLimit(rateLimit.retryAfter!)
      );
      return;
    }
...
_Nhập chủ đề của bạn bên dưới:_
  ...
  async handleCancel(msg: TelegramBot.Message): Promise<void> {
    ...
    await telegramService.sendMessage(
      chatId,
      "❌ Đã hủy quá trình hiện tại. Dùng /generate để bắt đầu lại."
    );
  }
...
_Hệ thống hoạt động ổn định_ ✨
    ...
    try {
      switch (conversation.step) {
        ...
        default:
          await telegramService.sendMessage(
            chatId,
            "Trạng thái không hợp lệ. Dùng /generate để bắt đầu lại."
          );
      }
    } catch (error: any) {
      ...
      await telegramService.sendMessage(
        chatId,
        telegramService.formatError(error.message)
      );
      conversationManager.resetConversation(userId, chatId);
    }
  }
  ...
  private async handleIdleState(
    ...
    await telegramService.sendMessage(
      chatId,
      "Dùng /generate để bắt đầu tạo nội dung, hoặc /help để xem hướng dẫn."
    );
  }
   ...
   */
  private async handleTopicInput(
    chatId: number,
    userId: string,
    topic: string
  ): Promise<void> {
    logger.info(
      `[User ${userId}] Topic input received: "${sanitizeForLog(topic)}"`
    );
    ...
    if (!validation.valid) {
      logger.warn(
        `[User ${userId}] Topic validation failed: ${validation.error}`
      );
    ...
    logger.debug(
      `[User ${userId}] Attempting to acquire lock for titles generation`
    );
    const lockAcquired = await conversationManager.tryAcquireLock(
      userId,
      chatId,
      "titles"
    );
    if (!lockAcquired) {
      const conversation = conversationManager.getConversation(userId, chatId);
      logger.warn(
        `[User ${userId}] Lock acquisition failed - already processing: ${conversation.processingTask}`
      );
      await telegramService.sendMessage(
        chatId,
    ...
    try {
      ...
      await telegramService.sendMessage(
        chatId,
        `✅ Chủ đề: *${escapeMarkdown(sanitizedTopic)}*\n\n🔄 Đang tạo 10 tiêu đề bài viết cho bạn...\n\n_Vui lòng chờ 10-20 giây..._`
      );

      logger.info(`[User ${userId}] Generating titles...`);
      const result = await contentService.generateTitles(
        sanitizedTopic,
        userId
      );
      logger.info(
        `[User ${userId}] Titles generated successfully using ${result.provider}`
      );
      ...
      logger.info(
        `[User ${userId}] Titles sent successfully - waiting for selection`
      );
    } catch (error: any) {
      logger.error(
        `[User ${userId}] Error during titles generation: ${error.message}`
      );
      ...
      if (error.message?.includes("timeout")) {
        const timeoutMinutes = Math.round(
          config.rateLimit.aiRequestTimeout / 60000
        );
        logger.warn(
          `[User ${userId}] Titles generation timed out after ${timeoutMinutes} minutes`
        );
        await telegramService.sendMessage(
          chatId,
      ...
      // Forward Gemini error to N8N instead of sending error message to user
      const conversation = conversationManager.getConversation(userId, chatId);
      await this.forwardErrorToN8N(chatId, userId, "titles", error, {
        topic: conversation.topic || sanitizedTopic,
      });

      // Don't send error message to user, just reset conversation
      conversationManager.resetConversation(userId, chatId);
    } finally {
   ...
   */
  private async handleTitleSelection(
    chatId: number,
    userId: string,
    selection: string
  ): Promise<void> {
    ...
    if (!validation.valid) {
      logger.warn(
        `[User ${userId}] Title selection validation failed: ${validation.error}`
      );
      await telegramService.sendMessage(chatId, `❌ ${validation.error}`);
      return;
    }
    ...
    if (isNaN(selectedIndex) || selectedIndex < 1 || selectedIndex > 10) {
      ...
      await telegramService.sendMessage(
        chatId,
        "❌ Số thứ tự không hợp lệ. Vui lòng chọn từ 1-10."
      );
      return;
    }
    ...
    if (
      !conversation.generatedTitles ||
      conversation.generatedTitles.length < selectedIndex
    ) {
      logger.error(
        `[User ${userId}] Title index out of bounds: ${selectedIndex} (available: ${conversation.generatedTitles?.length || 0})`
      );
      await telegramService.sendMessage(
        chatId,
        "❌ Tiêu đề không tồn tại. Vui lòng dùng /generate để bắt đầu lại."
      );
      conversationManager.resetConversation(userId, chatId);
      return;
    }
    ...
    const selectedTitle =
      conversation.generatedTitles[selectedIndex - 1]?.title;
    if (!selectedTitle) {
      logger.error(
        `[User ${userId}] Failed to retrieve title at index ${selectedIndex}`
      );
      await telegramService.sendMessage(
        chatId,
        "❌ Không thể lấy tiêu đề. Vui lòng dùng /generate để bắt đầu lại."
      );
      conversationManager.resetConversation(userId, chatId);
      return;
    }
    ...
    logger.info(
      `[User ${userId}] Title selected: "${sanitizeForLog(selectedTitle)}"`
    );

    // Try to acquire processing lock atomically
    logger.debug(
      `[User ${userId}] Attempting to acquire lock for outline generation`
    );
    const lockAcquired = await conversationManager.tryAcquireLock(
      userId,
      chatId,
    ...
    try {
      ...
      await telegramService.sendMessage(
        chatId,
        `✅ Bạn đã chọn tiêu đề: *${escapeMarkdown(selectedTitle)}*\n\n🔄 Đang tạo dàn ý chi tiết\\.\\.\\.`
      );

      logger.info(`[User ${userId}] Generating outline...`);
      const result = await contentService.generateOutline(
        selectedTitle,
        userId
      );
      logger.info(
        `[User ${userId}] Outline generated successfully using ${result.provider}`
      );
...
_Dàn ý chi tiết đã được gửi đến webhook để tiếp tục xử lý._
      ...
      // Forward full outline data to N8N for tracking (if configured)
      if (config.n8nTrackingUrl && config.n8nTrackingUrl.trim() !== "") {
        ...
        logger.debug(
          `[User ${userId}] N8N tracking URL not configured, skipping forward`
        );
      }
      ...
      logger.error(
        `[User ${userId}] Error during outline generation: ${error.message}`
      );
      logger.error(`Error stack: ${error.stack}`);
      ...
      if (error.message?.includes("timeout")) {
        ...
        logger.warn(
          `[User ${userId}] Outline generation timed out after ${timeoutMinutes} minutes`
        );
        await telegramService.sendMessage(
          chatId,
      ...
      // Forward Gemini error to N8N instead of sending error message to user
      const conversation = conversationManager.getConversation(userId, chatId);
      await this.forwardErrorToN8N(chatId, userId, "outline", error, {
        topic: conversation.topic,
        selectedTitle: conversation.selectedTitle || selectedTitle,
      });

      // Release lock on error
      logger.debug(
        `[User ${userId}] Releasing outline generation lock due to error`
      );
      conversationManager.releaseLock(userId, chatId);
  ...
  private async generateArticle(
    ...
    logger.info(
      `[User ${userId}] Article generation requested for title: "${sanitizeForLog(title)}"`
    );

    // Try to acquire processing lock atomically
    logger.debug(
      `[User ${userId}] Attempting to acquire lock for article generation`
    );
    const lockAcquired = await conversationManager.tryAcquireLock(
      userId,
      chatId,
    ...
    try {
      ...
      const articleResult = await contentService.generateArticle(
        title,
        outline,
        userId
      );
      logger.info(
        `[User ${userId}] Article generated successfully using ${articleResult.provider}`
      );
      logger.info(
        `[User ${userId}] Article stats - words: ${articleResult.data.article.wordCount}, tokens: ${articleResult.tokensUsed}`
      );

      // Calculate cost
      const cost =
        articleResult.inputTokens && articleResult.outputTokens
          ? calculateCost(
              articleResult.provider as "gemini",
              articleResult.inputTokens,
              articleResult.outputTokens
            )
          : 0;
...
_Bài viết đầy đủ đã được gửi đến webhook._
      ...
      let contentHtml = "";
      try {
        contentHtml = await convertMarkdownForWordPress(
          articleResult.data.article.content
        );
        logger.info(
          `[User ${userId}] Markdown conversion successful (${contentHtml.length} chars)`
        );
      } catch (conversionError: any) {
        logger.error(
          `[User ${userId}] Markdown conversion failed: ${conversionError.message}`
        );
        // Continue with empty HTML - webhook can fall back to markdown
      }

      // Forward full article data to N8N for tracking (if configured)
      if (config.n8nTrackingUrl && config.n8nTrackingUrl.trim() !== "") {
      ...
      logger.error(
        `[User ${userId}] Error during article generation: ${error.message}`
      );
      logger.error(`Error stack: ${error.stack}`);
      ...
      if (error.message?.includes("timeout")) {
        ...
        logger.warn(
          `[User ${userId}] Article generation timed out after ${timeoutMinutes} minutes`
        );
        await telegramService.sendMessage(
          chatId,
          `⏰ *Timeout*\n\nYêu cầu vượt quá thời gian cho phép (${timeoutMinutes} phút).\n\nVui lòng thử lại hoặc chọn chủ đề ngắn gọn hơn.`
        ...
        await this.forwardErrorToN8N(chatId, userId, "article", error, {
          topic: conversation.topic,
          selectedTitle: title,
          outline: outline,
        });
